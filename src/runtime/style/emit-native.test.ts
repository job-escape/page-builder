...
describe("nativeBox", () => {
  it("draws an inside stroke as a border, which is what a border is", () => {
    const { style, overlayStroke } = nativeBox(
      boxFromProps({ shadow: "inset 0 0 0 1px #000000" }),
    );
  ...
  it("hands an outside stroke back as an overlay, because a border would move the box", () => {
    const { style, overlayStroke } = nativeBox(
      boxFromProps({ shadow: "0 0 0 2px #a86565" }),
    );
  ...
  it("insets a centre stroke by half its width", () => {
    const box = boxFromProps({
      shadow: "0 0 0 1.5px #000000, inset 0 0 0 1.5px #000000",
    });
    expect(nativeBox(box).overlayStroke).toMatchObject({
      width: 3,
      inset: 1.5,
    });
  });
  ...
  it("returns a gradient as a component's props, never as a style key", () => {
    const box = boxFromProps({
      fill: "linear-gradient(90deg, #ffffff 0%, #000000 100%)",
    });
  ...
  it("resolves a token against the mode being rendered", () => {
    ...
    expect(
      nativeBox(box, { tokens, mode: "light" }).style.backgroundColor,
    ).toBe("#2563eb");
    expect(nativeBox(box, { tokens, mode: "dark" }).style.backgroundColor).toBe(
      "#60a5fa",
    );
  });
...
describe("nativeSize", () => {
  it("turns fill into flex rather than a percentage React Native cannot honour", () => {
    expect(nativeSize("fill", "width")).toEqual({
      flexGrow: 1,
      flexShrink: 1,
      flexBasis: 0,
    });
...
describe("nativeSize, told which way the parent flows", () => {
  ...
  it("stretches across a column rather than growing down it", () => {
    expect(nativeSize("fill", "width", "column")).toEqual({
      alignSelf: "stretch",
    });
  });
  ...
  it("grows along a row and stretches across it", () => {
    expect(nativeSize("fill", "width", "row")).toEqual({
      flexGrow: 1,
      flexShrink: 1,
      flexBasis: 0,
    });
    expect(nativeSize("fill", "height", "row")).toEqual({
      alignSelf: "stretch",
    });
  });
...
describe("nativeColor from CSS", () => {
  const palette = {
    "Mode 1": { "text.primary": "#fafafa", "fg.on-brand": "#ffffff" },
  };

  it("resolves a var() text colour instead of handing React Native a string it draws black", () => {
    expect(
      nativeColor("var(--text-primary)" as never, { tokens: palette }),
    ).toBe("#fafafa");
  });

  it("finds a token whose name has a hyphen inside a segment", () => {
    expect(
      nativeColor("var(--fg-on-brand)" as never, { tokens: palette }),
    ).toBe("#ffffff");
  });

  it("keeps literals and token references as they were", () => {
    expect(nativeColor("#112233", { tokens: palette })).toBe("#112233");
    expect(nativeColor({ $token: "text.primary" }, { tokens: palette })).toBe(
      "#fafafa",
    );
  });
});
